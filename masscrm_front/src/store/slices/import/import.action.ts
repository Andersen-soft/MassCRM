...
import { formatSearchList, HTTPFile, download } from 'src/utils';
import {
  IContactDownload,
  IFilter,
  IImportedData,
...
import { setNotification, setErrorHTTPRequestAction } from 'src/store/slices';
...
const loadResponsibleSearchListAction = (actionParams: IFilter) => async (
  ...
  try {
    ...
    const responsibleSearchList = formatSearchList(
      data?.filter(({ roles: { administrator } }: IUser) => !administrator) ||
        []
    );
...
const uploadFileAction = (file: File) => async (dispatch: Dispatch) => {
  try {
    ...
    const {
      data: { data, file_size, file_name }
    } = await HTTP.post('import/upload-file/', formData);
    dispatch(
      uploadFile({
        docInfo: data,
        fileInfo: { name: file.name, size: file_size, uploadName: file_name }
      })
    );
...
export const setShowCountImportContactsAction = createAction(
  'SET_SHOW_COUNT_IMPORT_CONTACTS_ACTION'
);

export const setShowCountImportContacts = (perPage: number) => async (
  dispatch: Dispatch,
  state: () => IStoreState
) => {
  const {
  ...
  await dispatch(
    setShowCountImportContactsAction({ meta: { ...meta, per_page: perPage } })
  );
};
